...
CREATE TABLE `investimento` (
  ...
  `nome_personalizado` varchar(100) DEFAULT NULL,
  `valor_aplicado` decimal(10,2) NOT NULL,
  `rentabilidade_anual` decimal(5,2) NOT NULL,
  `data_aplicacao` date NOT NULL,
  `data_vencimento` date DEFAULT NULL,
  PRIMARY KEY (`id_investimento`),
  KEY `id_usuario` (`id_usuario`),
  KEY `id_tipo_investimento` (`id_tipo_investimento`),
  CONSTRAINT `investimento_ibfk_1` FOREIGN KEY (`id_usuario`) REFERENCES `usuario` (`id_usuario`),
  CONSTRAINT `investimento_ibfk_2` FOREIGN KEY (`id_tipo_investimento`) REFERENCES `tipoinvestimento` (`id_tipo_investimento`)
